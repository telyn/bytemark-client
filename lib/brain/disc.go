...
func (d Disc) PrettyPrint(wr io.Writer, detail prettyprint.DetailLevel) error {
	tmpl := `{{ define "disc_sgl" }}{{ .Label }} - {{ gibtib .Size }}, {{ .StorageGrade }} grade{{ template "hasbackups" . }}{{ template "restoring" . }}{{ end }}
...
{{ define "restoring" }}{{ if eq .StorageGrade "iceberg" }} (restore in progress){{ end }}{{ end }}`
