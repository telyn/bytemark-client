...
func TestVirtualMachineNameSliceFlag(t *testing.T) {
	sf := flags.VirtualMachineNameSliceFlag{}	
	err := sf.Set("staples.stapler.photocopier")
	if err != nil {
		t.Errorf("got error from Set(): %s", err)
	}
	if len(sf) != 1 {
		t.Errorf("Expected len(VirtualMachineNameSliceFLag) to be 1, got %d",
		len(sf))
	}
	...
	cfg.When("GetGroup").Return(lib.GroupName{Group: "default-group", Account:
	"default-account"})
	...
	if sf.String() != "staples.stapler.photocopier" {
		t.Errorf("Expected %q, got %q", "staples.stapler.photocopier", sf.String())
	}
