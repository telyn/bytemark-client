...
func (cmds *CommandSet) HelpForConfig() cmd.ExitCode {
...
func (cmds *CommandSet) Config(args []string) cmd.ExitCode {
	if len(args) == 0 {
	...
	case "set":
		...
		if err != nil {
			if e, ok := err.(*cmd.ConfigReadError); ok {
				fmt.Fprintf(os.Stderr, "Couldn't read the old value of %s - %v\r\n", e.Name, e.Err)
			} else {
				fmt.Fprintf(os.Stderr, "Couldn't read the old value of %s - %v\r\n", variable, err)
			}
			return cmd.E_CANT_READ_CONFIG
		}
		...
		if err != nil {
			...
			return cmd.E_CANT_WRITE_CONFIG
		}
