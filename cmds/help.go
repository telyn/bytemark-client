...
func (cmds *CommandSet) HelpForHelp() util.ExitCode {
	...
	log.Log("    create dis<c|k>[s] [--account <account>] [--group <group>] [--size <size>] [--grade <storage grade>] <virtual machine> [<disc specs>] - if ambiguous, berate user")
	log.Log("    create vm [flags] <name> [<cores> [<memory> [<disc specs>]]] - creates a vm with the given name and the following flags")
	log.Log("    delete account <account>")
	log.Log("    delete dis<c|k> [--force] [---purge] <virtual machine> <disc label>")
	log.Log("    delete group <group>")
	log.Log("    delete vm [--force] [---purge] <virtual machine>")
	log.Log("    debug [--junk-token] [--auth] <method> <path> - Make an HTTP request to the given path on the current endpoint.")
	log.Log("    debug config - output the current config as json to debug Config's internal state")
	...
	log.Log("    list discs <virtual machine> - lists the discs in the given VM, with their size and ids")
	...
	log.Log("    resize dis<c|k> [--size <size>] <virtual machine> [<resize spec>] - resize to size. if ambiguous, berate user.")
	...
	log.Log("    undelete vm <virtual machine>")
