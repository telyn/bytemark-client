...
func (cmds *CommandSet) HelpForDelete() cmd.ExitCode {
...
func (cmds *CommandSet) DeleteVM(args []string) cmd.ExitCode {
...
func (cmds *CommandSet) DeleteGroup(args []string) cmd.ExitCode {
	flags := cmd.MakeCommonFlagSet()
	...
	if len(group.VirtualMachines) > 0 {
		if *recursive {
			...
			if cmd.PromptYesNo("Are you sure you want to continue?") {
			...
			return cmd.E_WONT_DELETE_NONEMPTY
		}
	}
	return cmd.ProcessError(cmds.bigv.DeleteGroup(name))
...
func (cmds *CommandSet) UndeleteVM(args []string) cmd.ExitCode {
